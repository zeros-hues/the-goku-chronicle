  ...
  return (
    <Card index={index}>
      ...
      <p style={{ display: "flex", alignItems: "baseline", gap: 3 }}>
        ...
        {unit && (
          <span style={{ fontFamily: "var(--font-martian-mono)", fontSize: 16, fontWeight: 400, color: "var(--text-muted)" }}>
            {unit}
          </span>
        )}
      </p>
...
export default function DashboardView({
...
}: { entries: Entry[]; members: Member[]; clients: Client[] }) {
  void clients;
  ...
  const overtimeData = useMemo(() => {
    ...
    return members.filter(m => memberDays[m.id]).map(m => {
      ...
      const totalOvertime = overtimeDays.reduce((s, h) => s + Math.max(0, h - 8), 0);
  ...
  const { busiestDay, avgDailyHours } = useMemo(() => {
    ...
    return { busiestDay: busiest ? `${busiest[0]} (${busiest[1]}h)` : "—", avgDailyHours: avg.toFixed(1) };
  }, [entries, totalHours]);
    ...
    <div className="space-y-8 pt-8">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mt-4">
          <ChartCard title="Hours by Project" index={4}>
          ...
          <ChartCard title="Hours by Team Member" index={5}>
      ...
      {/* Daily stacked */}
      <section>
        <ChartCard title="Daily Hours by Person" index={6}>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mt-4">
          <ChartCard title="Billing Split" index={7}>
          ...
          <ChartCard title="Quick Stats" index={8}>
            <div className="space-y-4">
              {[
                { label: "Busiest day",    value: busiestDay },
                { label: "Avg daily",      value: `${avgDailyHours}h` },
                    ...
                    <td style={{ padding: "14px 24px", color: "var(--text-primary)", fontFamily: "var(--font-instrument-sans)", fontWeight: 500 }}>
                      <div className="flex items-center gap-3">
                        <div style={{
                          ...
                          fontSize: 10, fontWeight: 700, fontFamily: "var(--font-martian-mono)", flexShrink: 0,
                        }}>
                          {row.initials}
                        </div>
                        {row.name}
